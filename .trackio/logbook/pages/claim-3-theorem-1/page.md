...
---
<!-- trackio-cell
{"type": "markdown", "id": "cell_5d57a888aa17", "created_at": "2026-07-22T03:55:52+00:00", "title": "C3: G⁺ captures d-separations — VERIFIED (exact)"}
-->
**Theorem 1:** for any disjoint A,B,C ⊆ X, A^(T) ⊥_d B^(T) | C^(T) in the evolutionary model G^(T) (with selection S^(<T) conditioned) ⟺ A ⊥_d B | C in the clique-augmented DAG G⁺.

**VERIFIED exactly:** across 3 base selection graphs and ALL disjoint (A,B,C) triples (12 per graph), the d-separation verdicts in G^(T) and G⁺ **match with 0 mismatches**. d-separation computed via the standard ancestral-moral-graph algorithm.


---
<!-- trackio-cell
{"type": "code", "id": "cell_c0701fa79b6f", "created_at": "2026-07-22T03:55:55+00:00", "title": "Re-run all-claim verification", "command": ["uv", "run", "python", "repro/src/verify.py"], "exit_code": 0, "duration_s": 0.12}
-->
````bash
...
````output
C1 model: True {'trait_within_gen': True, 'trait_to_selection': True, 'exogenous_to_trait': True, 'inheritance': True}
C2 induced deps: True
   G1 base=2 G+=2 extra=0
   G2 base=2 G+=3 extra=1
   G3 base=2 G+=3 extra=1
C3 d-sep capture (Theorem 1): True
   G1 tested=12 mismatches=0 holds=True
   G2 tested=12 mismatches=0 holds=True
   G3 tested=12 mismatches=0 holds=True
C4 PC soundness: True
   G1 G+edges=2 removed=1 wrongly_removed=0
   G2 G+edges=3 removed=0 wrongly_removed=0
   G3 G+edges=3 removed=0 wrongly_removed=0
C5 multi-env: True {'anS': ['X1', 'X2'], 'Gplus_has_selection_clique': True, 'consistent_across_environments': True, 'VERDICT': 'VERIFIED'}
